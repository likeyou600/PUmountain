...
@section('other')
    <div class="row">
        ...
        @php
            $quantity = count($borrow_other);
        @endphp
        @for ($time = 0; $time < $quantity; $time++)
            @php
                $category = $borrow_other[$time]->items_category;
                $value = $borrow_other[$time]->items_quantity;
                $id = $borrow_other[$time]->items_id;
                $pic = $borrow_other[$time]->items_picture;
            @endphp
            <div class="col-sm-3 col-6 text-center pointimagestyle phoneborrow">
                <div class="card PUcardtop">
                    @if ($value > 0)
                        <img src="../picture/PUmountain/borrow/other/{{ $pic }}" class="card-img-top">
