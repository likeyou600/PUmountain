...
@section('mat')
    <div class="row">
        ...
        @php
            $quantity = count($borrow_mat);
        @endphp
        @for ($time = 0; $time < $quantity; $time++)
            @php
                $category = $borrow_mat[$time]->items_category;
                $value = $borrow_mat[$time]->items_quantity;
                $id = $borrow_mat[$time]->items_id;
                $pic = $borrow_mat[$time]->items_picture;
            @endphp
            <div class="col-sm-3 col-6 text-center pointimagestyle phoneborrow">
                <div class="card PUcardtop">
                    @if ($value > 0)
                        <img src="../picture/PUmountain/borrow/mat/{{ $pic }}" class="card-img-top">
