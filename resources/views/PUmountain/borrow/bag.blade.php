...
@section('bag')
    <div class="row">
        ...
        @php
            $quantity = count($borrow_bag);
        @endphp
        @for ($time = 0; $time < $quantity; $time++)
            @php
                $category = $borrow_bag[$time]->items_category;
                $value = $borrow_bag[$time]->items_quantity;
                $id = $borrow_bag[$time]->items_id;
                $pic = $borrow_bag[$time]->items_picture;
            @endphp
            <div class="col-sm-3 col-6 text-center pointimagestyle phoneborrow">
                <div class="card PUcardtop">
                    @if ($value > 0)
                        <img src="../picture/PUmountain/borrow/bag/{{ $pic }}" class="card-img-top">
