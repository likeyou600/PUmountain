...
@section('burner')
    <div class="row">
        ...
        @php
            $quantity = count($borrow_burner);
        @endphp
        @for ($time = 0; $time < $quantity; $time++)
            @php
                $category = $borrow_burner[$time]->items_category;
                $value = $borrow_burner[$time]->items_quantity;
                $id = $borrow_burner[$time]->items_id;
                $pic = $borrow_burner[$time]->items_picture;
            @endphp
            <div class="col-sm-3 col-6 text-center pointimagestyle phoneborrow">
                <div class="card PUcardtop">
                    @if ($value > 0)
                        <img src="../picture/PUmountain/borrow/burner/{{ $pic }}" class="card-img-top">
